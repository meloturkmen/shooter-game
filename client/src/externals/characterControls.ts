...
export class CharacterController {
     ...
     */
    public setAnimationGroups(agMap: unknown) {
    ...
    public setAnimationRanges(arMap: unknown) {
        this._isAG = false;
    ...
    public setActionMap(inActMap: ActionMap): string {
        ...
        const ccActionNames: string[] = Object.keys(this._actionMap);
        for (const ccActionName of ccActionNames) {
            const ccActData = this._actionMap[ccActionName];
    ...
    public getActionMap(): ActionMap {
        const map: ActionMap = new ActionMap();

        const keys: string[] = Object.keys(this._actionMap);
        for (const key of keys) {
            const actDataI = this._actionMap[key];

            if (!(actDataI instanceof ActionData)) continue;
            if (!actDataI.exist) continue;

            const actDataO: ActionData = map[actDataI.id];
    ...
    public getSettings(): CCSettings {
        const ccs: CCSettings = new CCSettings();
    ...
    public enableBlending(n: number) {
        if (this._isAG) {
            const keys: string[] = Object.keys(this._actionMap);
            for (const key of keys) {
                const act = this._actionMap[key];
                if (!(act instanceof ActionData)) continue;
                if (act.exist) {
                    const ar: AnimationGroup = act.ag;
    ...
    public disableBlending() {
        if (this._isAG) {
            const keys: string[] = Object.keys(this._actionMap);
            for (const key of keys) {
                const anim = this._actionMap[key];
                if (!(anim instanceof ActionData)) continue;
                if (anim.exist) {
                    const ar: AnimationGroup = anim.ag;
                    for (const ta of ar.targetedAnimations) {
    ...
    private checkAGs(agMap: unknown) {
        const keys: string[] = Object.keys(this._actionMap);
        for (const key of keys) {
            const anim = this._actionMap[key];
            if (!(anim instanceof ActionData)) continue;
    ...
    private _containsAG(node: Node, ags: AnimationGroup[], fromRoot: boolean) {
        ...
        for (const ag of ags) {
            const tas: TargetedAnimation[] = ag.targetedAnimations;
            for (const ta of tas) {
            ...
            if (this._prevActData !== actData) {
                if (actData.exist) {
                    ...
                    if (this._isAG) {
                        ...
                        const a: Animatable = this._skeleton.beginAnimation(actData.name, actData.loop, actData.rate);
    ...
    private _calcJumpDist(speed: number, dt: number): number {
        ...
        const js: number = speed - this._gravity * this._jumpTime;
    ...
    private _rotateAV2C() {
        if (this._hasCam)
            if (this._mode != 1) {
                const ca = (this._hasCam) ? (this._av2cam - this._camera.alpha) : 0;
                if (this._noRot) {
                    switch (true) {
    ...
    private _pickedMeshes: AbstractMesh[] = []
    ...
    private _handleObstruction() {
        ...
        if (this._makeInvisible) {
            ...
            if (pis.length > 0) {
                ...
                for (const pm of this._prevPickedMeshes) {
    ...
    private _addkeylistener() {
        const canvas: HTMLCanvasElement = this._scene.getEngine().getRenderingCanvas();
    ...
    public setAvatar(avatar: Mesh, faceForward: boolean = false): boolean {
        ...
        const rootNode = this._root(avatar);
     ...
     * @param faceForward 
     */
    constructor(avatar: Mesh, camera: ArcRotateCamera, scene: Scene, actionMap?: unknown, faceForward = false) {
        ...
        const success = this.setAvatar(avatar, faceForward);
        if (!success) {
